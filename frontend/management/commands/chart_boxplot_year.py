...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
            make_option('--year',
                    dest='year',
                    default=2014,
                    help='A season year to display e.g. 2014 will be Winter start 2013 to Autumn end 2014'),
            make_option('--filter',
                    dest='filter',
                    default=None,
            ...
            make_option('--channel',
                    dest='channel_name',
                    default=None,
                    help='A channel to plot'),
            make_option('--file',
                    dest='filename',
                    default=None,
                    help='A filename where to save the plot'),
            make_option('--samescale',
                    action="store_true",
                    dest='samescale',
                    default=False,
                    help='Show all plots on same Y scale'),
    )


    def handle(self, *args, **options):
        
        if options['channel_name'] == None:
            print "You must select a chanel using --channel"
            return
