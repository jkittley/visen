...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Load all the channels from their models
        channels = []
        ...
        if options['date_list']:
            for date_str in options['date_list']:
                try:
                    newtm = datetime.strptime(date_str,'%Y-%m-%d')
                    if newtm not in sample_periods:
                        sample_periods.append( newtm )
                except ValueError:
        ...
        for sample_period in sample_periods:
            for channel in channels:
                tmpmax = SensorReading.objects.filter(sensor=profile.sensor, timestamp__range=(sample_period, sample_period+timedelta(days=1)), channel=channel).aggregate(Max('value'))
        ...
        for sample_period in sample_periods:
            ...
            for ch in data_vals:
                if counter > 0: 
                    ...
                    ax = ax1
                ax.step(x, data_vals[ch], colors[counter], label=ch.name)
                ax.set_ylabel(ch.name.capitalize()+' Usage', color=colors[counter])
                ax.tick_params(axis='x', labelsize=10, color=colors[counter])
                for tl in ax.get_yticklabels():
                    tl.set_color(colors[counter])
                counter += 1
